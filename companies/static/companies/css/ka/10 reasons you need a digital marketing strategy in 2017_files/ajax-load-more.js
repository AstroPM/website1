	...
	$.ajaxloadmore = function(el) {
		...
		var AjaxLoadMore = {}, 
			...
			$posts_per_page = $content.data('posts-per-page'),
                        $meta_key = $content.data('meta_key'),
                        $cat = $content.data('cat'),
                        $member_level = $content.data('member_level'),
                        $role = $content.data('role'),
                        $search_prod_title = $content.data('search_prod_title');
		...
		$el.append('<div class="'+$prefix+'btn-wrap"><p class="ajax_resource"></p><button id="load-more" class="'+$prefix+'load-more-btn more btn green-btn dark">SHOW MORE RESOURCES</button></div>');
							...
							$el.slideDown(speed, 'alm_easeInOutQuad', function() {
								...
								if ($data.length < $posts_per_page) {
									...
									$button.addClass('done');
                                             ...
                                             if($( window ).width() > 768){
                                                    var articleBox = $('#grid .article-box');
                                                    articleBox.each(function(){
                                                        var articleBoxHeight = $(this).height();
                                                        if(articleBoxHeight > $("#box_height").val()){
                                                        $("#box_height").val(articleBoxHeight);
                                                        }
                                                    });
                                                    if($("#box_height").val() > 0 ){
                                                        $('#grid .article-box').css('height', $("#box_height").val());
                                                    }
                                                    }
                                                if($(".resource-matching").html() != ''){
                                                $(".ajax_resource").html($(".resource-matching").html());
                                                $(".resource-matching").remove();
                                                }
                                                } else {
